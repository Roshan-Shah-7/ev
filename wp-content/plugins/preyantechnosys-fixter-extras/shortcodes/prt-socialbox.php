		...
		if( is_array($socialdata) && count($socialdata)>0 ){
			foreach( $socialdata as $data ){
				if( !empty($data->servicename) ){
					...
					$return .= '<a class="prt-socialbox-icon-link prt-socialbox-icon-link-'.$data->servicename.'" target="_blank" href="'.$servicelink.'" data-tooltip="'.$servicename.'"><i class="prt-fixter-icon-'.$data->servicename.'"></i><span class="prt-link">'.$data->servicename.'</span></a>';
					$return .= '</div>';
