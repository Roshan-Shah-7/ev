...
function preyantechnosys_fixter_cpt_tm_team(){

	// Getting Options
	$fixter_theme_options = get_option('fixter_theme_options');
	
	$team_type_title          = ( !empty($fixter_theme_options['team_type_title']) ) ? $fixter_theme_options['team_type_title'] : 'Team Members' ;
	...
	$team_type_slug           = ( !empty($fixter_theme_options['team_type_slug']) ) ? $fixter_theme_options['team_type_slug'] : 'team-member' ;
	
	$team_group_title          = ( !empty($fixter_theme_options['team_group_title']) ) ? $fixter_theme_options['team_group_title'] : 'Team Groups' ;
	$team_group_title_singular = ( !empty($fixter_theme_options['team_group_title_singular']) ) ? $fixter_theme_options['team_group_title_singular'] : 'Team Group' ;
	$team_cat_slug           = ( !empty($fixter_theme_options['team_cat_slug']) ) ? $fixter_theme_options['team_cat_slug'] : 'team-group' ;
	...
	add_action('do_meta_boxes', 'preyantechnosys_fixter_tm_team_featured_image_box');
	function preyantechnosys_fixter_tm_team_featured_image_box() {
...
add_action( 'init', 'preyantechnosys_fixter_cpt_tm_team', 8 );
 ...
 *  Meta Box: Team
 */
if ( ! function_exists( 'preyantechnosystmte_fixter_tm_team_metabox_options' ) ) {
function preyantechnosystmte_fixter_tm_team_metabox_options( $options ) {
	...
	$team_extra_details_lines  = ( !empty($fixter_theme_options['team_extra_details_lines']) ) ? $fixter_theme_options['team_extra_details_lines'] : array() ;
	...
	$list_array = array(
		...
		array(
			'id'     		=> 'team_details_line_company_image',
			'type'    		=> 'image',
			'title'   		=> esc_attr__('Company Logo', 'tmte'),
			'after'  		=> '<div class="cs-text-muted"><br>'.__("(Optional) Please set company logo", 'tmte').'</div>',
		)
	...
	if( is_array($team_extra_details_lines) && count($team_extra_details_lines) > 0 ){
		...
		foreach( $fixter_theme_options['team_extra_details_lines'] as $key=>$val ){
			
	
			$this_array = array();
			$this_array['id']    = 'team_extra_details_line_'.$key;
			$this_array['type']  = 'text';
			$this_array['title'] = '<span class="prt-admin-team-list-icon"></span> &nbsp; '. esc_attr__($val['team_extra_details_line_title'], 'fixter');
	...
	$options[]    = array(
		...
		'title'     => sprintf( esc_attr__("Fixter: %s's Details", 'tmte'), $team_type_title_singular ),
	...
	$options[]    = array(
		...
		'title'     => sprintf( esc_attr__("Fixter: %s's Social Links", 'tmte'), $team_type_title_singular ),
		'post_type' => 'tm_team_member', // only here is important
		'context'   => 'normal',
		'priority'  => 'default',
		'sections'  => array(
			...
			array(
				...
				'fields' => array(
					array(
						...
						'info'            => esc_attr__('Add your social services here. Also you can reorder the Social Links as per your choice. Just drag and drop items to reorder and save settings', 'fixter'),
						'button_title'    => esc_attr__('Add New Social Links', 'fixter'),
						'accordion_title' => 'social_icons_list_icon',
						'fields'          => array(
							array(
								'id'            => 'social_icons_list_icon',
								'type'          => 'select',
								'title'         =>  esc_attr__('Social Sevice', 'fixter'),
								'options'  		=> array(
													...
													'rss'        => esc_attr__('RSS', 'fixter' ),
												),
								'class'         => 'chosen',
								'default'       => 'twitter',
								'after'  		=> '<div class="cs-text-muted"><br>'.__('Select Social service from here', 'fixter').'</div>',
							),
							array(
								'id'     		=> 'social_icons_list_link',
								'type'    		=> 'text',
								'title'   		=> esc_attr__('Link to Social service selected above', 'fixter'),
...
add_filter( 'cs_metabox_options', 'preyantechnosystmte_fixter_tm_team_metabox_options' );
