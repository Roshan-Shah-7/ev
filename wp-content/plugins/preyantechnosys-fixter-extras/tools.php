...
function preyantechnosys_pagination( $wp_query_data=false ){
	...
	$paginateLinks = paginate_links( array(
		...
		'type'      => 'array',
		'prev_text' => '<i class="prt-fixter-icon-arrow-left"></i> <span class="prt-hide prt-pagination-text prt-pagination-text-prev">' . esc_attr__( 'Previous page', 'fixter' ) . '</span>',
		'next_text' => '<span class="prt-hide prt-pagination-text prt-pagination-text-next">' . esc_attr__( 'Next page', 'fixter' ) . '</span> <i class="prt-fixter-icon-arrow-right"></i>',
	) );
			...
			switch( $meta_id ){
				...
				case 'date':
					...
					if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
						...
						$return .= sprintf( '<span class="prt-meta-line posted-on"><i class="prt-fixter-icon-clock"></i> <span class="screen-reader-text prt-hide">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
				...
				case 'author':
					...
					if ( 'post' === get_post_type() ) {
						...
						$return .= sprintf( '<span class="prt-meta-line byline"><i class="prt-fixter-icon-user"></i> <span class="author vcard"><span class="screen-reader-text prt-hide">%1$s </span>%2$s</span></span>',
				...
				case 'comment':
					...
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
						$return .= '<span class="prt-meta-line comments-link"><i class="prt-fixter-icon-comments-smiley"></i> ';
						ob_start();
						comments_popup_link( esc_attr__( 'Leave a comment', 'fixter' ) );
				...
				case 'tag':
					...
					if ( !empty($tags_list) ) {
						...
						$return .= sprintf( '<span class="prt-meta-line tags-links"><i class="prt-fixter-icon-tag"></i> <span class="screen-reader-text prt-hide" prt-hide>%1$s </span>%2$s</span>',
	...
	if( !empty($type) ){
		...
		if( !empty($return) ){
			...
			switch( $type ){
				...
				case 'phone':
					$return = '<div class="prt-team-list-title prt-skincolor"><i class="prt-fixter-icon-phone"></i> '. esc_attr__('Phone','fixter') .'</div>
						<div class="prt-team-list-value"><a href="tel:' . $return . '">' . $return . '</a></div>';
					break;
					
				case 'email':
					$return = '<div class="prt-team-list-title prt-skincolor"><i class="prt-fixter-icon-mail"></i> '. esc_attr__('Email','fixter') .'</div>
				...
				case 'website':
					...
					$return = '<div class="prt-team-list-title prt-skincolor"><i class="prt-fixter-icon-world"></i> '. esc_attr__('Website','fixter') .'</div>
...
function preyantechnosys_portfolio_next_prev_btn() {
	...
	if( is_singular('tm_portfolio') ){
		...
		$return = get_the_post_navigation( array(
			'next_text' => '<span class="meta-nav prt-hide" aria-hidden="true">' . __( 'Next', 'fixter' ) . '</span> ' .
				'<span class="screen-reader-text prt-hide">' . __( 'Next post:', 'fixter' ) . '</span> ' .
				'<span class="post-title prt-hide">%title</span>',
			'prev_text' => '<span class="meta-nav prt-hide" aria-hidden="true">' . __( 'Previous', 'fixter' ) . '</span> ' .
				'<span class="screen-reader-text prt-hide">' . __( 'Previous post:', 'fixter' ) . '</span> ' .
				'<span class="post-title prt-hide">%title</span>',
		) );
...
function preyantechnosys_portfolio_detailsbox(){
	...
	if( isset($pf_details_line) && is_array($pf_details_line) && count($pf_details_line)>0 ){
		foreach( $pf_details_line as $key=>$val ){
			...
			if( !empty($val['pf_details_line_title']) ){ $row_title = sprintf( __('%s', 'fixter'), $val['pf_details_line_title'] ); }
...
function preyantechnosys_portfoliobox_media_link( $default_icon='prt-fixter-icon-search', $video_icon='prt-fixter-icon-video', $audio_icon='prt-fixter-icon-music-alt', $slider_icon='prt-fixter-icon-gallery-1' ){

	$icon_code='<i class="prt-fixter-icon-search"></i>';
...
function preyantechnosys_global_portfolio_template_list( $for_vc=false ){
	$return = array(
		'overlay'    => esc_attr__('Content overlay on image view', 'fixter'),
		'top-image'  => esc_attr__('Top Image and Bottom Content view', 'fixter'),
	);
...
function preyantechnosys_global_blog_template_list( $for_vc=false ){
	$return = array(
		"top-image"       => esc_attr__("Top image and bottom content (default)", "fixter"),
		"left-image"      => esc_attr__('Left image and right content ', "fixter"),
		"right-image"     => esc_attr__('Right image and left content ', "fixter"),
		"content-overlay" => esc_attr__('Content overlay on image', "fixter"),
	);
...
function preyantechnosys_global_team_member_template_list( $for_vc=false ){
	$return = array(
		"overlay"    => esc_attr__("Overlay content on Image (Default)", "fixter"),
		"left-image" => esc_attr__("Left Image and Right Content", "fixter"),
	);
...
function preyantechnosys_global_client_template_list( $for_vc=false ){
	$return = array(
		'simple-logo'    => esc_attr__('Simple Logo view', 'fixter'),
		'separator-logo' => esc_attr__('Logo with Separator view', 'fixter'),
		'boxed-logo'     => esc_attr__('Boxed view', 'fixter'),
		//'logo-with-title' => esc_attr__('Logo with Title view', 'fixter'),
	);
...
function preyantechnosys_header_links(){
	...
	if( function_exists('is_woocommerce') && $wc_header_icon==true ){
		...
		$return  .= '<span class="prt-header-icon prt-header-wc-cart-link"><a href="' . $cart_url . '"><i class="prt-fixter-icon-cart"></i><span class="number-cart">&nbsp;</span></a></span>';
	}
	...
	if( $header_search==true ){
		$class   = '';
		$return .= '<span class="prt-header-icon prt-header-search-link"><a href="javascript:void(0);"><i class="prt-fixter-icon-search"></i></a></span>';
	}
...
function preyantechnosys_box_team_social_links(){
	...
	if( !empty($data) && is_array($data) && count($data)>0 ){
		...
		foreach($data as $social ){
			...
			$return .= '<li><a href="'. $social['social_icons_list_link'] .'" target="_blank"><i class="prt-fixter-icon-'. $social['social_icons_list_icon'] .'"></i><span class="prt-hide">'. $social_name .'</span></a></li>';
		}
...
function preyantechnosys_event_description(){ 
	$return   = '';
	$readMore = esc_attr__('See Event', 'fixter') . ' <i class="kwicon-fa-angle-right"></i>';
...
function preyantechnosys_comment_row_template($comment, $args, $depth){
    ...
    <div class="comment-meta commentmetadata">
		<?php printf( esc_attr__( '<cite class="prt-comment-owner fn">%s</cite> <span class="says">says:</span>', 'fixter' ), get_comment_author_link() ); ?>
...
function preyantechnosys_search_results_box_title( $post_type='post' ){
	...
	if( !empty($singular_name) ){
		...
		$return .= '<div class="prt-sresults-title-w"><h2 class="prt-sresults-title">' . sprintf(
				esc_attr__('Search results for %s','fixter'),
