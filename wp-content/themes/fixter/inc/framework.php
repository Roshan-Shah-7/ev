 ...
 */
if( ! function_exists( 'fixter_cs_framework_init' ) ) {
function fixter_cs_framework_init(){
	// Codestar framework config
	defined('CS_OPTION'          ) or define('CS_OPTION',           'fixter_theme_options' );
...
add_action( 'init', 'fixter_cs_framework_init', 1 );
add_action( 'admin_init', 'fixter_cs_framework_init', 1 );
			...
			if( isset($section['fields']) && is_array($section['fields']) ){
				foreach( $section['fields'] as $field ){
					if( isset($field['id']) && isset($field['default']) ){
						...
						$fixter_theme_options[$field_id] = $field_val;
					}
				}
...
function preyantechnosys_cs_wp_enqueue_scripts($elements) {
	...
	if( count($all_fonts)>0 ){
		...
		foreach( $all_fonts as $font_name=>$font_values ){
			...
			$google_font_names[] = $font_name;
		}
		$google_font_list = urlencode( implode('|',$google_font_list) );
		$custom_fonts = array( 'Satoshi', 'Magnita' );
		
		if( !is_admin() ){
			if(!in_array( $google_font_names, $custom_fonts ) ){
				wp_enqueue_style( 'preyantechnosys-cs-google-fonts', esc_url( '//fonts.googleapis.com/css?family=Poppins' ), array(), null );
			}
			else {
...
function preyantechnosys_get_font_option_values($elements){
	...
	if( is_array($elements) && count($elements)>0 ){
		...
		foreach( $elements as $element ){
			$val = ( isset($fixter_theme_options[$element]) ) ? $fixter_theme_options[$element] : array() ;
