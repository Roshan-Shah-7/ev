...
function preyantechnosys_fixter_cpt_tm_service(){

	$fixter_theme_options = get_option('fixter_theme_options');
	
	$service_type_title          = ( !empty($fixter_theme_options['service_type_title']) ) ? $fixter_theme_options['service_type_title'] : 'Service' ;
	...
	$service_type_slug           = ( !empty($fixter_theme_options['service_type_slug']) ) ? $fixter_theme_options['service_type_slug'] : 'service' ;
	
	$service_cat_title          = ( !empty($fixter_theme_options['service_cat_title']) ) ? $fixter_theme_options['service_cat_title'] : 'Service Categories' ;
	$service_cat_title_singular = ( !empty($fixter_theme_options['service_cat_title_singular']) ) ? $fixter_theme_options['service_cat_title_singular'] : 'Service Category' ;
	$service_cat_slug           = ( !empty($fixter_theme_options['service_cat_slug']) ) ? $fixter_theme_options['service_cat_slug'] : 'service-category' ;
...
add_action( 'init', 'preyantechnosys_fixter_cpt_tm_service', 8 );
 ...
 *  Meta Boxes: Service
 */
if ( ! function_exists( 'preyantechnosys_fixter_tm_service_metabox_options' ) ) {
function preyantechnosys_fixter_tm_service_metabox_options( $options ) {
...
$service_type_title          = ( !empty($fixter_theme_options['service_type_title']) ) ? $fixter_theme_options['service_type_title'] : 'Service' ;
	$service_type_title_singular = ( !empty($fixter_theme_options['service_type_title_singular']) ) ? $fixter_theme_options['service_type_title_singular'] : 'Service' ;
	// Default options - Team Member details	$list_array = array(		 array (			"id"    => "service_details_boxdesc",			"type"  => "textarea",			"title" => '<span class="tm-admin-service-list-icon"></span> &nbsp; '.__('Short Details', 'tmte'),		)	);
	$post_id = !empty($_GET['post']) ? $_GET['post'] : get_the_ID() ;		$options[]    = array(		'id'        => 'preyantechnosys_services_details',		'title'     => sprintf( esc_attr__("TTM Medical: %s's Details", 'tmte'), $service_type_title_singular ),		'post_type' => 'tm_service', // only here is important		'context'   => 'normal',		'priority'  => 'default',		'sections'  => array(			array(				'name'   => 'preyantechnosys_service_list_data',				'fields' => array(					array(						'id'        => 'tm_service_info',						'type'      => 'fieldset',						//'title'     => esc_attr__('List Values','tmte'),						'fields'    => $list_array,										),				),			),		),	);
	
	$list_array    = array();
	$options_array = array();
	if( isset($fixter_theme_options['service_details_line']) && is_array($fixter_theme_options['service_details_line']) && count( $fixter_theme_options['service_details_line'] ) > 0 ){
		foreach( $fixter_theme_options['service_details_line'] as $key=>$val ){
			...
			$option_array = array(
				...
				'type'       => 'text',
				'title'      => '<span class="prt-admin-service-list-icon"> <i class="'. $icon_class .'"></i></span> &nbsp; '. esc_attr__($val['service_details_line_title'], 'fixter'),
			);
	...
	$options[]    = array(
		...
		'title'     => esc_attr__('Fixter: Featured Image / Video / Slider', 'tmte'),
		'post_type' => 'tm_service', // only here is important
		'context'   => 'normal',
		'priority'  => 'default',
		'sections'  => array(
