...
function preyantechnosys_shared_social_list(){
	...
	$sociallist = array(
		...
		'youtube'      => esc_attr__('YouTube', 'fixter'),
		'flickr'       => esc_attr__('Flickr', 'fixter'),
		'facebook'     => esc_attr__('Facebook', 'fixter'),
		'linkedin'     => esc_attr__('LinkedIn', 'fixter'),
		'gplus'        => esc_attr__('Google+', 'fixter'),
		'yelp'         => esc_attr__('Yelp', 'fixter'),
		'dribbble'     => esc_attr__('Dribbble', 'fixter'),
		'pinterest'    => esc_attr__('Pinterest', 'fixter'),
		'podcast'      => esc_attr__('Podcast', 'fixter'),
		'instagram'    => esc_attr__('Instagram', 'fixter'),
		'xing'         => esc_attr__('Xing', 'fixter'),
		'vimeo'        => esc_attr__('Vimeo', 'fixter'),
		'vk'           => esc_attr__('VK', 'fixter'),
		'houzz'        => esc_attr__('Houzz', 'fixter'),
		'issuu'        => esc_attr__('Issuu', 'fixter'),
		'google-drive' => esc_attr__('Google Drive', 'fixter'),
	);
...
function preyantechnosys_pagination( $wp_query_data=false ){
	...
	$paginateLinks = paginate_links( array(
		...
		'type'      => 'array',
		'prev_text' => '<span class="prt-hide prt-pagination-text prt-pagination-text-prev">' . esc_attr__( 'Prev', 'fixter' ) . '</span>',
		'next_text' => '<span class="prt-pagination-text prt-pagination-text-next">' . esc_attr__( 'Next', 'fixter' ) . '</span>',
	) );
	...
	if( $paginateLinks!=NULL ){
		...
		$return .= paginate_links( array(
			...
			'current'   => max( 1, get_query_var('paged') ),
			'total'     => $wp_query->max_num_pages,
			'prev_text' => '<span class="prt-pagination-text prt-pagination-text-prev">' . esc_attr__( 'Prev', 'fixter' ) . '</span>',
			'next_text' => '<span class="prt-pagination-text prt-pagination-text-next">' . esc_attr__( 'Next', 'fixter' ) . '</span>',
		) );
...
function preyantechnosys_get_option( $option, $inner_option='' ){
	...
	if( !is_array($fixter_theme_options) ){
		
		if( is_multisite() ){
			$fixter_theme_options = get_site_option('fixter_theme_options');
		} else {
	...
	if( isset($fixter_theme_options[$option]) ){
		$return = $fixter_theme_options[$option];
	} else {
...
function preyantechnosys_get_all_registered_sidebars(){
	...
	$return = array( '' => esc_attr__('Default', 'fixter') );
...
/**
 * Custom template tags for Fixter
 *
...
if ( ! function_exists( 'fixter_comment_nav' ) ) :
/**
 * Display navigation to next/previous comments when applicable.
 *
 * @since Fixter 1.0
 */
function fixter_comment_nav() {
	// Are there comments to navigate through?
	if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
	?>
	<nav class="navigation comment-navigation">
		<h2 class="screen-reader-text"><?php esc_attr_e( 'Comment navigation', 'fixter' ); ?></h2>
		<div class="nav-links">
			<?php
				if ( $prev_link = get_previous_comments_link( esc_attr__( 'Older Comments', 'fixter' ) ) ) :
					printf( '<div class="nav-previous">%s</div>', $prev_link );
				endif;

				if ( $next_link = get_next_comments_link( esc_attr__( 'Newer Comments', 'fixter' ) ) ) :
...
if ( ! function_exists( 'fixter_entry_meta' ) ) :
/**
 * Prints HTML with meta information for the categories, tags.
 *
 * @since Fixter 1.0
 */
function fixter_entry_meta( $metafor="blogbox" ) {
			...
			switch( $meta_id ){
				...
				case 'date':
					...
					if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
						...
						$return .= sprintf( '<span class="prt-meta-line posted-on"><i class="ti-calendar"></i> <span class="screen-reader-text prt-hide">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
							esc_attr_x( 'Posted on', 'Used before publish date.', 'fixter' ),
				...
				case 'author':
					...
					if ( 'post' === get_post_type() ) {
						...
						$return .= sprintf( '<span class="prt-meta-line byline"><i class="ti-user"></i> <span class="byauthor">'. esc_attr__( 'By', 'fixter' ) .'</span> <span class="author vcard"><span class="screen-reader-text prt-hide">%1$s </span>%2$s</span></span>',
							esc_attr_x( 'Author', 'Used before post author name.', 'fixter' ),
							$author
						);
						}
					}
				...
				case 'comment':
					...
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
						...
						ob_start();
						comments_popup_link( esc_attr__( '0 Comments', 'fixter' ) );
				...
				case 'cat':
					...
					if ( !empty($categories_list) ) {
						...
						$return .= sprintf( '<span class="prt-meta-line cat-links"><i class="prt-fixter-icon-category"></i> <span class="screen-reader-text prt-hide">%1$s </span>%2$s</span>',
							esc_attr_x( 'Categories', 'Used before category names.', 'fixter' ),
							$categories_list
						);
					}
				...
				case 'tag':
					...
					if ( !empty($tags_list) ) {
						...
						$return .= sprintf( '<span class="prt-meta-line tags-links"><i class="prt-fixter-icon-tag-2"></i> <span class="screen-reader-text prt-hide">%1$s </span>%2$s</span>',
							esc_attr_x( 'Tags', 'Used before tag names.', 'fixter' ),
							$tags_list
						);
					}
 ...
 */
if( ! function_exists('fixter_categorized_blog') ){
function fixter_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'fixter_categories' ) ) ) {
		...
		set_transient( 'fixter_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so fixter_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so fixter_categorized_blog should return false.
		return false;
	}
}
}
...
function preyantechnosys_blogbox_comment_count( $echo = false ) {
	...
	if( comments_open() ){
		...
		$return  .= '<div class="preyantechnosys-blogbox-comment"><i class="prt-fixter-icon-comment"></i> '. $comments .'</div>';
	}
	return $return;
}
}
	...
	if( !empty($type) ){
		...
		if( !empty($return) ){
			...
			switch( $type ){
				...
				case 'phone':
					$return = '<div class="prt-team-list-title"><i class="kw_fixter flaticon-call-1"></i> '. esc_attr__('Phone','fixter') .' :</div>
						<div class="prt-team-list-value"><a href="tel:' . $return . '">' . $return . '</a></div>';
					break;
					
				case 'email':
					$return = '<div class="prt-team-list-title"><i class="prt-fixter-icon-mail"></i> '. esc_attr__('Email','fixter') .' :</div>
				...
				case 'website':
					...
					$return = '<div class="prt-team-list-title"><i class="prt-fixter-icon-world"></i> '. esc_attr__('Website','fixter') .' :</div>
...
/**
 * Getting company loog for review
 */
if ( !function_exists( 'preyantechnosys_get_reviewlogo_media' ) ){
function preyantechnosys_get_reviewlogo_media( $post_id='', $size='full'){
	...
	if( !empty($post_id) ){
		...
		$company_image = ( !empty($post_meta['tm_team_info']['team_details_line_company_image']) ) ? trim($post_meta['tm_team_info']['team_details_line_company_image']) : '' ;



		$return .= wp_get_attachment_image( $company_image, 'thumbnail' );		
	}
	
	// Adding wrapper
	if( !empty($return) ){
		$return = '<div class="preyantechnosys-box-img-right">' . $return . '</div>';
	}
...
function preyantechnosys_create_icon_from_data( $data, $i_tag_only=false ){
	...
	if( isset($data['library']) && isset($data['library_fontawesome']) && isset($data['library_linecons']) && isset($data['library_themify']) ){
		...
		$library_kw_fixter = ( !empty($data['library_kw_fixter']) ) ? $data['library_kw_fixter'] : '';
		...
		if( !empty(${'library_'.$library}) && ${'library_'.$library}!='empty' ){
			...
			$return = do_shortcode('[prt-icon type="'.$library.'" icon_kw_fixter="'.$library_kw_fixter.'" icon_fontawesome="'.$library_fontawesome.'" icon_linecons="'.$library_linecons.'" icon_themify="'.$library_themify.'"]');
...
function preyantechnosys_portfolio_next_prev_btn() {
	...
	if( is_singular('tm_portfolio') ){
		...
		$return = get_the_post_navigation( array(
			'next_text' => '<span class="meta-nav" aria-hidden="true">' . esc_attr__( 'Next', 'fixter' ) . '</span> ' .
				'<span class="screen-reader-text prt-hide">' . esc_attr__( 'Next post:', 'fixter' ) . '</span> ' .
				'<span class="post-title prt-hide">%title</span>',
			'prev_text' => '<span class="meta-nav" aria-hidden="true">' . esc_attr__( 'Previous', 'fixter' ) . '</span> ' .
				'<span class="screen-reader-text prt-hide">' . esc_attr__( 'Previous post:', 'fixter' ) . '</span> ' .
				'<span class="post-title prt-hide">%title</span>',
		) );
...
function preyantechnosys_portfolio_detailsbox(){
	...
	if( isset($pf_details_line) && is_array($pf_details_line) && count($pf_details_line)>0 ){
		foreach( $pf_details_line as $key=>$val ){
			...
			if( !empty($val['pf_details_line_title']) ){ $row_title = sprintf( esc_attr__('%s ', 'fixter'), $val['pf_details_line_title'] ); }
...
function preyantechnosys_portfoliofull_detailsbox(){
	...
	if( isset($pf_details_line) && is_array($pf_details_line) && count($pf_details_line)>0 ){
		...
		$return .= '<li class="prt-pf-details-date psocial-block">'.preyantechnosys_social_share_box($post_type='portfolio').'</li>';
...
function preyantechnosys_portfoliobox_media_link( $default_icon='prt-fixter-icon-resize-full-alt', $video_icon='prt-fixter-icon-video', $audio_icon='prt-fixter-icon-music-alt', $slider_icon='prt-fixter-icon-gallery-1' ){

	$icon_code='<i class="prt-fixter-icon-resize-full-alt"></i>';
...
function preyantechnosys_gallerystyle_portfoliobox_media_link( $default_icon='prt-fixter-icon-search', $video_icon='prt-fixter-icon-video', $audio_icon='prt-fixter-icon-music-alt', $slider_icon='prt-fixter-icon-gallery-1' ){

	$icon_code='<i class=" prt-fixter-icon-search-2"></i>';
	if( !empty($default_icon) ){
		$icon_code='<i class="' . $default_icon . '"></i>';
	}
...
function preyantechnosys_box_portfolio_viewproject_btn(){
	...
	if( !empty($btn_text) && !empty($btn_link) ){
		$return .= '
		<div class="prt-viewproject-btn-wrapper">
			<div class="vc_btn3-container vc_btn3-left">
			<a class="prt-vc_general prt-vc_btn3 prt-vc_btn3-size-md prt-vc_btn3-shape-square prt-vc_btn3-style-flat prt-vc_btn3-weight-no prt-vc_btn3-color-skincolor" href="'. esc_url($btn_link) .'" target="_self">'. esc_attr($btn_text) .'</a>
			</div><!-- .vc_btn3-container.vc_btn3-left -->
		</div><!-- .prt-team-viewproject-btn-wrapper -->  ';
	}
	return $return;
}
}
...
function preyantechnosys_get_post_format_icon( $post_id='' ){
	...
	if( $post_type=='post' ){
		...
		$return = '<i class="prt-fixter-icon-' . $icon_class . '"></i>';
...
function preyantechnosys_get_boxes( $cpt='blog', $vars=array() ){
	...
	$allword  = (!empty($vars['allword'])) ? $vars['allword'] : esc_attr__('All', 'fixter');
...
function preyantechnosys_global_portfolio_template_list( $for_vc=false ){
	$return = array(	
			array(
				'label'	=> esc_attr__('Style 2','fixter'),
				'value'	=> 'style2',
...
function preyantechnosys_global_service_template_list( ){
	$return = array(	
				array(
					'label'	=> esc_attr__('Service Box - Style 1','fixter'),
					'value'	=> 'styleone',
...
function preyantechnosys_global_iconbox_template_list( $for_vc=false ){
	$return = array(	
				array(
					'label'	=> esc_attr__('Iconbox Box - Style 1','fixter'),
					'value'	=> 'styleone',
...
function preyantechnosys_global_blog_template_list( $for_vc=false ){
	$return = array(	
				array(
					'label'	=> esc_attr__('Top image and bottom content (default)','fixter'),
					'value'	=> 'style1',
				...
				array(
					'label'	=> esc_attr__('Style 3','fixter'),
					'value'	=> 'style3',
					'thumb'	=> get_template_directory_uri() . '/inc/images/blogbox-style3.png',
				),
				array(
					'label'	=> esc_attr__('Left image and right content','fixter'),
					'value'	=> 'left-image',
...
function preyantechnosys_global_team_member_template_list( $for_vc=false ){
	$return = array(	
				array(
					'label'	=> esc_attr__('Style 1','fixter'),
					'value'	=> 'style1',
...
function preyantechnosys_global_testimonial_template_list( $for_vc=false ){
		$return = array(	
				array(
					'label'	=> esc_attr__('Testimonial Style 1','fixter'),
					'value'	=> 'style-1',
...
function preyantechnosys_blogbox_readmore_text(){
	...
	$return             = esc_attr__('Read More', 'fixter');
...
function preyantechnosys_servicebox_readmore_text(){
	...
	$return             = esc_attr__('More Details', 'fixter');
...
function preyantechnosys_entry_icon() {
	...
	$iconCode .= '<i class="prt-fixter-icon-'.$icon.'"></i>';
...
function preyantechnosys_blogbox_single_meta( $option='date', $args=array() ) {
	...
	switch($option){
		...
		case 'category' :
			...
			$categories_list = get_the_category_list( esc_attr_x( ', ', 'Used between list items, there is a space after the comma.', 'fixter' ) );
			if ( !empty($categories_list) ) {
...
function preyantechnosys_titlebar_classes(){
	...
	if( $breadcrum_border_bottom == 'yes' ){
		$reurn[] = 'prt-breadcrumb-border-bottom';
	}
...
function preyantechnosys_titlebar_title(){
	...
	if( is_singular() || is_home() ){  // single page, single post and single cpt
		...
		if( is_home() ){
			...
			$title = esc_attr__( 'Blog', 'fixter' );  // Setting for Titlebar title
		}	
		...
		$adv_tbar_catarc = ( !empty($adv_tbar_catarc) ) ? esc_attr($adv_tbar_catarc.' %s') : esc_attr__('Category Archives: %s', 'fixter') ;
		$title = sprintf(
			$adv_tbar_catarc,
		...
		$adv_tbar_tagarc = ( !empty( $adv_tbar_tagarc ) ) ? esc_attr($adv_tbar_tagarc.' %s') : esc_attr__('Tag Archives: %s','fixter') ;
		$title = sprintf(
			$adv_tbar_tagarc,
		...
		if( is_tax('tm_team_group') || is_tax('tm_portfolio_category') ){
			...
			global $wp_query;
			$adv_tbar_postclassified = ( !empty($adv_tbar_postclassified) ) ? esc_attr($adv_tbar_postclassified.' %s') : esc_attr__('Posts classified under: %s', 'fixter') ;
		...
		$adv_tbar_authorarc = ( !empty( $adv_tbar_authorarc ) ) ? esc_attr($adv_tbar_authorarc.' %s') : esc_attr__('Author Archives: %s', 'fixter');
		...
		$title    = sprintf( esc_attr__( 'Search Results for %s', 'fixter' ), '<span class="prt-titlebar-heading prt-tbar-search-title">' . get_search_query() . '</span>' );


	} else if( is_404() ){ // 404
		$error404_big_text = preyantechnosys_get_option('error404_big_text');
		$title = esc_attr__( 'Error Page', 'fixter' ); 
		...
		if( function_exists('tribe_is_month') && tribe_is_month() && !is_tax() ) { // The Main Calendar Page
			...
			global $wp_query;
			$title = esc_attr__( 'Events on: ', 'fixter' ). date('F j, Y', strtotime($wp_query->query_vars['eventDate']));
		...
		// BBPress section
			...
			$title = esc_attr__( 'Forum', 'fixter' );
		} else if( is_post_type_archive() ){
			$title = post_type_archive_title('', false);
		} else if ( is_day() ){
			$title = sprintf( esc_attr__( 'Daily Archives: %s', 'fixter' ), '<span>' . get_the_date() . '</span>' );
		} elseif( is_month() ){
			$title = sprintf( esc_attr__( 'Monthly Archives: %s', 'fixter' ), '<span>' . get_the_date( esc_attr_x( 'F Y', 'monthly archives date format', 'fixter' ) ) . '</span>' );
		} elseif( is_year() ){
			$title = sprintf( esc_attr__( 'Yearly Archives: %s', 'fixter' ), '<span>' . get_the_date( esc_attr_x( 'Y', 'yearly archives date format', 'fixter' ) ) . '</span>' );
		} else {
			if( function_exists('is_bbpress') && is_bbpress() ) {
				$title = esc_attr__( 'Forum', 'fixter' );
			} else {
				$title = esc_attr__( 'Archives', 'fixter' );
			}
		};
	...
	if( preyantechnosys_fbar_show()==true ){
		...
		<span class="preyantechnosys-fbar-btn ' . preyantechnosys_sanitize_html_classes(preyantechnosys_fbar_btn_classes()) . ' preyantechnosys-fbar-bg-' . preyantechnosys_sanitize_html_classes( preyantechnosys_get_option('fbar_btn_bg_color') ) . '">
			<a href="#" class="preyantechnosys-fbar-btn-link">
				...
				<span class="prt-hide">' . esc_attr__('Open', 'fixter') . '</span>
			</a>
		</span>';
	}
	return $return;
...
function preyantechnosys_header_links(){
	...
	// Floating bar icon
	if( preyantechnosys_fbar_show()==true && $show_topbar==true ){
		$return .= '
		<span class="preyantechnosys-fbar-btn ' . preyantechnosys_sanitize_html_classes(preyantechnosys_fbar_btn_classes()) . ' preyantechnosys-fbar-bg-' . preyantechnosys_sanitize_html_classes( preyantechnosys_get_option('fbar_btn_bg_color') ) . '">
			<a href="#" class="preyantechnosys-fbar-btn-link">
				' . preyantechnosys_fbar_open_icon() . '
	...
	if( function_exists('is_woocommerce') && $wc_header_icon==true ){
		...
		$return  .= '<span class="prt-header-icon prt-header-wc-cart-link"><a href="' . $cart_url . '"><i class="prt-fixter-icon-shopping-cart"></i><span class="number-cart">' . esc_attr($count) . '</span></a></span>';
	}
		
	// Search icon
	if( $header_search==true ){
		$class   = '';
		$return .= '<div class="prt-header-icon prt-header-search-link"><a href="#"><i class="flaticon-loupe"></i></a></div>';
	}
...
function preyantechnosys_header_floatingbar_icon(){
	...
	if( $return!='' ){
		$return = '<div class="prt-header-flotingbar-icon '.preyantechnosys_fbar_icon_classes().' ' . $class . '">'. $return .'</div>';
	}
...
function preyantechnosys_headerclass(){
	...
	$slider_quicktext2 = preyantechnosys_get_option('header_slider_righttext');
 ...
 *  Floating Bar button classes
 */
if( !function_exists('preyantechnosys_fbar_icon_classes') ){
function preyantechnosys_fbar_icon_classes(){
		
	$fbar_icon_position = preyantechnosys_get_option('fbar-icon-position');
	
	
	$return = array();
	
	if( !empty($fbar_icon_position) ){
		$return[] = 'prt-fbar-icon-position-' . $fbar_icon_position;
	}
...
function preyantechnosys_footer_row_class( $row='first' ){
	$class = '';
	global $fixter_theme_options;
	$secondary_footer_border = preyantechnosys_get_option('secondary_footer_border');
	$socialbar_show = preyantechnosys_get_option('socialbar_show');
	
	// BG color
	if( !empty($fixter_theme_options[$row.'_footer_bg_color']) ){
		$class .= ' prt-bg prt-bgcolor-'.sanitize_html_class($fixter_theme_options[$row.'_footer_bg_color']);
	}
	
	// Text color
	if( !empty($fixter_theme_options[$row.'_footer_text_color']) ){
		$class .= ' prt-textcolor-'.sanitize_html_class($fixter_theme_options[$row.'_footer_text_color']);
	}
	
	// If bg image is there
	if( !empty($fixter_theme_options[$row.'_footer_bg_all']['image']) ){
	...
	// border type
	if( !empty($fixter_theme_options[$row.'_footer_bordertype']) ){
		$class .= ' prt-bordertype-'.sanitize_html_class($fixter_theme_options[$row.'_footer_bordertype']);
	}
	
	//widget border color
	if( $row =='first' && !empty($fixter_theme_options['border_between_footerwidget_area']) ){
		$class .= ' prt-footerrow-sep'.sanitize_html_class($fixter_theme_options['border_between_footerwidget_area']);
	}
	
	if( $row =='first' && !empty($fixter_theme_options['first_footer_widget_seperator']) ){
		$class .= ' prt-widgetsep-'.sanitize_html_class($fixter_theme_options['first_footer_widget_seperator']);
	}
	if( $row =='second' && !empty($fixter_theme_options['border_between_footerwidget_area']) ){
		$class .= ' prt-footerrow-sep'.sanitize_html_class($fixter_theme_options['border_between_footerwidget_area']);
	}
	// Top border for Footer Copyright area 
	if( $row=='bottom' && !empty($fixter_theme_options['border_above_copyright_area']) ){
		$class .= ' prt-bordercolor-'.sanitize_html_class($fixter_theme_options['border_above_copyright_area']);
	}
	if( $row =='second' && !empty($fixter_theme_options['second_footer_widget_seperator']) ){
		$class .= ' prt-widgetsep-'.sanitize_html_class($fixter_theme_options['second_footer_widget_seperator']);
	}
	if( !empty($secondary_footer_border) && $secondary_footer_border == 'yes' ){
		$secondary_footer_border = "yes";
	}
	if( $secondary_footer_border == 'yes' ){
		$class .= ' prt-secondaryfooter-border';
	}
	if( !empty($socialbar_show) && $socialbar_show == 'yes' ){
		$class .= ' prt-footersocial-yes';
	} else {
		$class .= ' prt-footersocial-no';
	}
	// Return data
	return $class;
...
function preyantechnosys_box_team_social_links(){
	...
	if( !empty($data) && is_array($data) && count($data)>0 ){
		...
		foreach($data as $social ){
			...
			$return .= '<li class="'. $social_name .'"><a href="'. $social['social_icons_list_link'] .'" target="_blank"><i class="prt-fixter-icon-'. $social['social_icons_list_icon'] .'"></i><span class="prt-hide">'. $social_name .'</span></a></li>';
		}
...
function preyantechnosys_team_readmore_text(){
	...
	if( !empty($team_readmore_text) ){
		...
		$return = '<div class="preyantechnosys-team-readmore"><a href="'.get_permalink().'">'.$team_readmore_text.'</a></div>';
	}
 ...
 */
if( !function_exists('fixter_addhttp') ){
	function fixter_addhttp($url){
		if (!preg_match("~^(?:f|ht)tps?://~i", $url)){
			$url = "http://" . $url;
		}
		return $url;
...
function preyantechnosys_testimonial_featured_image($size='thumbnail'){
	...
	$return = ( !empty($featured_image) ) ? $featured_image : '<span class="preyantechnosys-icon-box"><i class="demo-icon prt-fixter-icon-quote-left"></i></span>';
...
function preyantechnosys_star_ratting(){
	...
	for ($i = 1; $i <= 5; $i++) {
		if( $star_ratings >= $i ){
			$return .= ' <i class="prt-fixter-icon-star-1 prt-active"></i> ';
		} else {
			$return .= ' <i class="prt-fixter-icon-star-empty-1"></i> ';
		}
	}
	return $return;
}
...
if( !function_exists('preyantechnosys_slider_quicktext2') ){
function preyantechnosys_slider_quicktext2(){
	$return		 = '';
	$headerstyle = preyantechnosys_get_headerstyle();
	$slider_quicktext2 = preyantechnosys_get_option('header_slider_righttext');
	...
	if( in_array( $headerstyle, $valid_headerstyle ) && !empty( $slider_quicktext2 ) ){
		$slider_quicktext2 = preyantechnosys_wp_kses( $slider_quicktext2 );
		
		$return      = '<div class="prt-quickdetails-area2">'.do_shortcode( $slider_quicktext2 ).'</div>';
	}
...
function preyantechnosys_revslider_array( $countonly=false ) {
	...
	if( $countonly==true ){
		...
		if( count($sliders)==0 ){
			$sliders[''] = esc_attr__('(No Slider Found)', 'fixter');
		}
		return $sliders;
	}
}
	...
	if ( function_exists('lsSliders') ) {
		...
		if( $countonly == true ){
			...
			if( count($slider_names) == 0 ){
				$slider_names[''] = esc_attr__('(No Slider Found)', 'fixter');
			}
			return $slider_names;
		}
...
function preyantechnosys_single_get_sidebar_value(){
	...
	$sidebar = $fixter_theme_options['sidebar_post'];
	...
	if( is_page() || is_singular() ){
		...
		if( !empty($fixter_theme_options['sidebar_'.$cpt]) ){
			$sidebar = $fixter_theme_options['sidebar_'.$cpt];
		}
	...
	if( is_search() ){
		$sidebar = $fixter_theme_options['sidebar_search'];
	}
...
function preyantechnosys_event_description(){ 
	$return   = '';
	$readMore = esc_attr__('See Event', 'fixter') . '';
...
function preyantechnosys_event_readmore(){ 
	$return   = '';
	$readMore = esc_attr__('View Details', 'fixter') . ' <i class="kwicon-fa-angle-right"></i>';
...
function preyantechnosys_comment_row_template($comment, $args, $depth){
    ...
    <<?php echo esc_attr($tag); ?> <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ) ?> id="comment-<?php comment_ID() ?>">
		...
		<?php if ( $comment->comment_approved == '0' ) : ?>
			 <em class="comment-awaiting-moderation"><?php esc_attr_e( 'Your comment is awaiting moderation.', 'fixter' ); ?></em>
			  <br />
		<?php endif; ?>
		<div class="comment-box">
			<div class="comment-meta commentmetadata">
				<?php printf( '<cite class="prt-comment-owner fn">%s</cite> <span class="says">' . esc_attr__('says:', 'fixter' ) . '</span>', get_comment_author_link() ); ?>
				
				<a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
				<?php
				printf( esc_attr__( '%1$s at %2$s', 'fixter' ), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( esc_attr__( '(Edit)', 'fixter' ), '  ', '' );
				?>
			</div>			
...
function preyantechnosys_author_social_links(){
	...
	foreach( $all_socials as $social_class => $social_link ){
		if( !empty($social_link) ){
			$return .= '<li><a href="'. $social_link .'" target="_blank"><i class="prt-fixter-icon-'. $social_class .'"></i><span class="prt-hide">'. ucwords($social_class) .'</span></a></li>';
		}
	}
		...
		switch( $post_type ){
			...
			case 'tm_service' :
				if( $curr_total_tm_service <= $total_tm_service ){
					...
					ob_start();
					wp_enqueue_style( 'fixter-service' );
...
function preyantechnosys_search_results_box_title( $post_type='post' ){
	...
	$small_link = '<small><a href="'. esc_url(get_home_url()).'?s='.get_search_query().'" class="label label-default"><i class="prt-fixter-icon-angle-left"></i> '.esc_attr__('Back to results','fixter').'</a></small>';
	if( empty($_GET['post_type']) ){
		$small_link = '<small><a href="'. esc_url(get_home_url()).'?s='.get_search_query().'&post_type=' . $post_type . '" class="label label-default">'.esc_attr__('View more','fixter').'</a></small>';
	}
...
function preyantechnosys_search_form(){
	...
	$cptList = array(
		'any'            => esc_attr__('All selections', 'fixter'),
		'page'           => esc_attr__('Pages', 'fixter'),
		'post'           => esc_attr__('Blog posts', 'fixter'),
	);
	...
	<div class="prt-sresult-form-wrapper">
					...
					<div class="prt-sresult-form-bottom-w">
						...
						<div class="prt-search-main-box clearfix">
                            <div class="prt-search-text"><strong>' . esc_attr__('Search in:','fixter') . '</strong></div>
							<div class="prt-search-select-box">
								' . $dropdown . '
                              <div class="prt-sresult-form-sbtbtn-wrapper">
								<input class="prt-sresult-form-sbtbtn" type="submit" value="' . esc_attr__('Search now','fixter') . '" />
							  </div>
						  </div>
...
function preyantechnosys_recent_posts( $post='' ){
	...
	$return .= '<li class="prt-recent-post-list-li">';
		...
		$return .= '<span class="post-date"><i class="ti-calendar"></i>' . fixter_entry_meta() . '</span>';
...
function preyantechnosys_footer_ctabox(){
	...
	if( !empty($fixter_theme_options['footer_cta_column_layout']) ){
...
function preyantechnosys_entry_date( $echo = true ) {
	if ( has_post_format( array( 'chat', 'status' ) ) ){
		$format_prefix = esc_attr_x( '%1$s on %2$s', '1: post format name. 2: date', 'fixter' );
	} else {
		$format_prefix = '%2$s';
	}
...
if ( ! function_exists( 'fixter_posttime_meta' ) ) :
function fixter_posttime_meta( $metafor="blogbox" ) {
...
function preyantechnosys_get_socialbar_links( $tooltip_position='top' , $tooltip='yes' ){
	...
	$return = '';
	if( !empty($fixter_theme_options['social_icons_list']) ){
		foreach( $fixter_theme_options['social_icons_list'] as $socialicon ){
			...
			$return .= '<li class="prt-social-' . $social_id . ' prt-socialbox-i-wrapper"><a class="prt-socialbox-icon-link prt-socialbox-icon-link-' . $social_id . ' ' . sanitize_html_class($class) . '" target="_blank" href="' . $href . '"><span class="frame-hover"></span><i class="prt-fixter-icon-' . $social_id . '"></i><span class="social_name">'.$social_name.'</span></a></li>' . "\n";
		}
	}
	...
	foreach( $socialArray as $key=>$value ){
		...
		if( $key == 'rss' ){
			if( isset($fixter_theme_options['rss']) && $fixter_theme_options['rss']=='1' ){
			...
			if( isset($fixter_theme_options[$key]) && trim($fixter_theme_options[$key])!='' ){
				$return .= '<li class="'.$key.' "><a target="_blank" href="'.esc_url($fixter_theme_options[$key]).'" data-tooltip="'.$value[1].'"><i class="prt-social-icon-'.$value[0].'"></i></a></li>';
			}
		}
	}
...
function ttm_element_container( $settings = array( 'position' => 'start', 'cpt' => 'blog', 'data' => array() ) ){
	...
	$tm_centerpadding			= ( !empty($settings['data']['prt-centerpadding']) ) ? trim($settings['data']['prt-centerpadding']) : '0' ;
	...
	$arrow_position   = (!empty($settings['data']['carousel_arrowposition'])) ? $settings['data']['carousel_arrowposition'] : '' ;
	...
	if( $position=='start' ){
		...
		$data_array[] = 'data-prt-nav="'.$arrow_position.'"';
		...
		if( $view_type=='carousel' ){	
			$class_array[] = 'prt-boxes-carousel-arrow-'.$arrow_position;
		}
		if( !empty($gap) ){
...
function preyantechnosys_post_like_meta( ) {
	...
	$tm_likeicon = ( isset($_COOKIE["preyantechnosys_likes_".get_the_ID()]) ) ? 'prt-fixter-icon-heart' : 'prt-fixter-icon-heart-empty' ;

	$tm_likedata = '';
	if( $tm_likes=='0' ){
		$tm_likedata = '<a href="#" class="prt-meta-like-link ' . esc_attr($tm_likeclass) . ' " id="pid-' . get_the_ID() . '"><i class="'. esc_attr($tm_likeicon).'"></i> <span>Likes:'.sprintf( esc_attr__( '%1$s ', 'fixter' ), $tm_likes ). '</span></a>';
	} else if( $tm_likes=='1' ){
		$tm_likedata = '<a href="#" class="prt-meta-like-link ' . esc_attr($tm_likeclass) . ' " id="pid-' . get_the_ID() . '"><i class="'. esc_attr($tm_likeicon).'"></i> <span>'.sprintf( esc_attr__( '%1$s ', 'fixter' ), $tm_likes ). '</span></a>';
	} else {
		$tm_likedata = '<a href="#" class="prt-meta-like-link ' . esc_attr($tm_likeclass) . ' " id="pid-' . get_the_ID() . '"><i class="'. esc_attr($tm_likeicon).'"></i> <span>'.sprintf( esc_attr__( '%1$s ', 'fixter' ), $tm_likes ). '</span></a>';
	}
	$like = sprintf( '<span class="prt-meta-line likes-link"><span class="screen-reader-text prt-hide">%1$s </span>%2$s</span><i class="prt-fixter-icon-heart-empty"></i>',
		esc_attr_x( ' ', 'Used before Likes in meta.', 'fixter' ),
		$tm_likedata
	);
...
add_option( 'fixterpurchase-verify', 'no', '', 'yes' );
...
if ( !function_exists( 'preyantechnosys_get_blogstyle5_desc' ) ){
function preyantechnosys_get_blogstyle5_desc( $for='' ){
	$blog_text_limit =85; 
	$blog_readmore_text = preyantechnosys_get_option('blog_readmore_text');
	$rbutton = esc_attr__('Read More', 'fixter');
	if( !empty($blog_readmore_text) ){
		$rbutton = esc_attr($blog_readmore_text);
	}
	...
	if( $blog_text_limit>0 ){
		...
		$content = substr($content,0, $blog_text_limit );
		$content = trim(preg_replace( '/\s+/', ' ', $content));
		$content = $content.''.'<a class="prt-rmorebutton" href="'.get_permalink().'">'.$rbutton.'</a>';
	}
	return $content;
}
}
...
if( !function_exists('preyantechnosys_socialbar_show') ){
function preyantechnosys_socialbar_show(){
	$socialbar_show = preyantechnosys_get_option('socialbar_show');
	$return    = false;
	
	if( $socialbar_show==true ){
		$return = true;
	}
...
/**
 *  Service long descritpion 
*/
    
if( !function_exists('preyantechnosys_box_service_smalldesc') ){
function preyantechnosys_box_service_smalldesc(){
    $return = '';
    $small_desc = '';
    ...
    $meta_data = get_post_meta( $post_id, 'preyantechnosys_services_details', true );

    if( !empty($meta_data['tm_service_info']['service_details_boxdesc']) ){
        $small_desc = $meta_data['tm_service_info']['service_details_boxdesc'];
    }
    if( !empty($small_desc) ){
        $return .= '<div class="tm-quickdetails">'. preyantechnosys_wp_kses($small_desc) .'</div>';
    }
