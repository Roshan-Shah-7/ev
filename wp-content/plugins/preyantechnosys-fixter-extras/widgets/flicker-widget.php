 ...
 * @since 1.0
 */
class fixter_flicker_widget extends WP_Widget {
	...
	function __construct() {
		$widget_style = array('classname'   => 'fixter_flicker_widget',
							  'description' => esc_attr__('Show photos from Flickr.','fixter') );
		...
		parent::__construct(
			'fixter_flicker_widget', // Base ID
			esc_attr__('Fixter Flickr Widget', 'fixter'), // Name
			$widget_style // Args
		);
	}
	...
	function widget( $args, $cur_instance ) {
		...
		echo '<div class="fixter_flicker_widget_wrapper">'; ?>
	...
	function form( $cur_instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php echo esc_attr__('Title', 'fixter'); ?>:</label>
...
register_widget( 'fixter_flicker_widget' );