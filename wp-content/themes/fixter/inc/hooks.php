...
function preyantechnosys_megamenu_override_default_theme($value) {
  ...
  if ( !isset($value['preyantechnosys-main-menu']['theme']) ) {
    $value['preyantechnosys-main-menu']['theme'] = 'fixter_theme_settings'; // change my_custom_theme_key to the ID of your exported theme
  }
...
function preyantechnosys_mmmenu_breakpoint( $options ){
	...
	if( empty($megamenu_themes) ){
		$megamenu_themes = array (
			'fixter_theme_settings' => 
				array (
				'title' => esc_attr__('Fixter Theme Settings', 'fixter'),
				'arrow_up' => 'dash-f343',
...
function preyantechnosys_pf_reset_like(){
    $screen = get_current_screen();
    if ( $screen->post_type == 'tm_portfolio' && isset($_GET['action']) && $_GET['action']=='edit' && !isset($_GET['taxonomy']) ){
        global $post;
        $postID = $_GET['post'];
        $resetVal = get_post_meta($postID, 'preyantechnosys_portfolio_like' ,true );

        if( $resetVal==true ){
            // Do reset processs now
            update_post_meta($postID, 'preyantechnosys_likes' , '0' ); // Setting ZERO
            update_post_meta($postID, 'preyantechnosys_portfolio_like' ,'' ); // Removing checkbox
        }
    }
    
}
add_action('current_screen', 'preyantechnosys_pf_reset_like');
...
function preyantechnosys_image_sizes(){
	...
	add_image_size( 'preyantechnosys-img-masonry-big', 412, 660, true ); // (cropped)
}
}
...
function preyantechnosys_single_skin_color(){
	...
	if( $post_id ){
		...
		if( !empty($page_meta['skincolor']) ){
			$fixter_theme_options['skincolor'] = esc_attr($page_meta['skincolor']);
		}
	}
}
}
				...
				if( substr($meta, 0, strlen($prefix) ) == $prefix ){
					...
					if( in_array($meta_name, $all_options) ){
						if( preyantechnosys_is_json($value[0]) && !is_numeric($value[0]) ){
							...
							$original_val = ( isset($fixter_theme_options[$meta_name]) ) ? $fixter_theme_options[$meta_name] : array() ;
							$final_val    = array_merge( $original_val, $final_val );
						} else {
							// string
							$final_val = $value[0];
						}
						$fixter_theme_options[$meta_name] = $final_val;
					}
...
/**
 * Flush out the transients used in {@see fixter_categorized_blog()}.
 *
 * @since Fixter 1.0
 */
if( !function_exists('fixter_category_transient_flusher') ){
function fixter_category_transient_flusher() {
	delete_transient( 'fixter_categories' );
}
}
add_action( 'edit_category', 'fixter_category_transient_flusher' );
add_action( 'save_post',     'fixter_category_transient_flusher' );
...
function preyantechnosys_add_inline_dynamic_css(){
	...
	// Minify CSS style
	if( !empty( $fixter_theme_options['minify'] ) && esc_attr($fixter_theme_options['minify'])==true ){
		$css = preyantechnosys_minify_css( $css );
	}
	
	// fixter main style
	wp_add_inline_style( 'fixter-main-style', $css );
...
function preyantechnosys_single_body_background(){
	...
	if( $post_id ){
		...
		if( isset($single_meta['custom_background_switcher']) && $single_meta['custom_background_switcher']==true && !empty($single_meta['custom_background']) ){
			...
			wp_add_inline_style( 'fixter-last-checkpoint', $css );
		}
...
if ( ! function_exists( 'fixter_excerpt_more' ) && ! is_admin() ) :
/**
 ...
 */
function fixter_excerpt_more( $more ) {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
		sprintf( esc_attr__( 'Continue reading %s', 'fixter' ), '<span class="screen-reader-text">' . get_the_title( get_the_ID() ) . '</span>' )
		);
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'fixter_excerpt_more' );
endif;
...
function preyantechnosys_body_classes($bodyClass){
	...
	if( isset($fixter_theme_options['titlebar_view']) && 
		($fixter_theme_options['titlebar_view'] == 'default' || $fixter_theme_options['titlebar_view'] == 'allleft' || $fixter_theme_options['titlebar_view'] == 'allright' ) &&
		isset($fixter_theme_options['breadcrumb_on_bottom']) &&
		$fixter_theme_options['breadcrumb_on_bottom'] == true
	){
		$bodyClass[] = 'prt-titlebar-bcrumb-bottom';
	}
	
	// Check if slider exists for header style classic
	if( !empty($fixter_theme_options['headerstyle']) && $fixter_theme_options['headerstyle']=='classic' && ( is_single() || is_page() ) ){
	...
	if( !empty($fixter_theme_options['headerstyle']) && $fixter_theme_options['headerstyle']=='classic-overlay' && ( is_single() || is_page() ) ){
		$singular_meta = get_post_meta( get_the_ID(), '_preyantechnosys_metabox_group', true );
		if( !empty($singular_meta['slidertype']) && $singular_meta['slidertype']=='revslider' && !empty($singular_meta['revslider']) ){
			$bodyClass[] = 'prt-pageslider-yes';
		}
	...
	if( !empty($fixter_theme_options['headerstyle']) ){
		$bodyClass[] = 'prt-headerstyle-'.esc_attr($fixter_theme_options['headerstyle']);
	}
	
	// check if dark background set for container.
	if( isset($fixter_theme_options['inner_background']['background-color']) && trim($fixter_theme_options['inner_background']['background-color'])!='' && preyantechnosys_check_dark_color(esc_attr($fixter_theme_options['inner_background']['background-color'])) ){
		$bodyClass[] = 'prt-dark-layout';
	}
	
	// show/hide separator line between links in dropdown menu
	if( isset($fixter_theme_options['dropdown_menu_separator']) && trim($fixter_theme_options['dropdown_menu_separator'])=='0' ){
		$bodyClass[] = 'prt-dropmenu-hide-sepline';
	}
	

	// Sticky Fotoer ON/OFF
	if( isset($fixter_theme_options['stickyfooter']) && $fixter_theme_options['stickyfooter']==true ){
		$bodyClass[] = 'preyantechnosys-sticky-footer';
	}
	
	// Header border bottom
	if( isset($fixter_theme_options['header_border_bottom']) && $fixter_theme_options['header_border_bottom']==true ){
		$bodyClass[] = 'preyantechnosys-header-borderbottom';
	}
	
	// Single Portfolio
	if( is_singular('portfolio') ){
		$portfolioView        = esc_attr($fixter_theme_options['portfolio_viewstyle']); // Global view
	...
	// Footer
	if( isset($fixter_theme_options['footer_style'])){
		$footerView        = esc_attr($fixter_theme_options['footer_style']); // Global view
	...
	if( isset($fixter_theme_options['show_topbar']) && $fixter_theme_options['show_topbar']==true ){
		$bodyClass[] = sanitize_html_class('preyantechnosys-topbar-yes');
	} else {
		$bodyClass[] = sanitize_html_class('preyantechnosys-topbar-no');
	}
	
	if( isset($fixter_theme_options['fbar_show']) && $fixter_theme_options['fbar_show']==true ){
		$bodyClass[] = sanitize_html_class('preyantechnosys-fbar-yes');
	} else {
		$bodyClass[] = sanitize_html_class('preyantechnosys-fbar-no');
	}
	
	if( isset($fixter_theme_options['button_link_hover']) && $fixter_theme_options['button_link_hover']=="border" ){
	...
	if( isset($fixter_theme_options['footer_cta_box']) && $fixter_theme_options['footer_cta_box']==true ){
		$bodyClass[] = sanitize_html_class('preyantechnosys-footer-cta-yes');
	}
	
	// Boxed / Wide
	if( isset($fixter_theme_options['layout']) && trim($fixter_theme_options['layout'])!='' ){
		if( $fixter_theme_options['layout']=='boxed' || $fixter_theme_options['layout']=='framed' || $fixter_theme_options['layout']=='rounded' ){
			$bodyClass[] = 'preyantechnosys-boxed';
		}
		if( $fixter_theme_options['layout']=='framed' || $fixter_theme_options['layout']=='rounded' ){
			$bodyClass[] = 'preyantechnosys-boxed-'.sanitize_html_class($fixter_theme_options['layout']);
		}
		
		$bodyClass[] = sanitize_html_class( 'preyantechnosys-'.trim($fixter_theme_options['layout']));
		if( $fixter_theme_options['layout']=='fullwide' ){
			if( isset($fixter_theme_options['full_wide_elements']['content']) && $fixter_theme_options['full_wide_elements']['content']=='1' ){
	...
	if( isset($fixter_theme_options['logoimg_retina']['url']) && $fixter_theme_options['logoimg_retina']['url']!=''){
	...
	$headerstyle_global = '';
	$headerstyle_page   = '';
	if( isset($fixter_theme_options['headerstyle']) && trim($fixter_theme_options['headerstyle'])!='' ){
		$headerstyle_global = sanitize_html_class($fixter_theme_options['headerstyle']);
	}
	if( is_page() ){
	...
	// One Page website
	if( isset($fixter_theme_options['one_page_site']) && $fixter_theme_options['one_page_site']==true ){
...
/** Post Like ajax **/
add_action('wp_ajax_preyantechnosys-portfolio-likes', 'preyantechnosys_ajax_callback' );
add_action('wp_ajax_nopriv_preyantechnosys-portfolio-likes', 'preyantechnosys_ajax_callback' );

function preyantechnosys_ajax_callback(){
	if(isset($_POST['likes_id'])){
		$post_id = str_replace('pid-', '', $_POST['likes_id']);
		echo preyantechnosys_update_like( $post_id );
	}
	exit;
}

function preyantechnosys_update_like( $post_id ){
	if(!is_numeric($post_id)) return;

	$return = '';
	$likes = get_post_meta($post_id, 'preyantechnosys_likes', true);
	if(!$likes){ $likes = 0; }
	$likes++;
	update_post_meta($post_id, 'preyantechnosys_likes', $likes);
	setcookie('preyantechnosys_likes_'.esc_attr($post_id), esc_attr($post_id), time()*20, '/');
...
add_filter( 'comment_form_fields', 'prt_comment_form_fields', 10, 1 );

function prt_comment_form_fields($comment_fields) {
