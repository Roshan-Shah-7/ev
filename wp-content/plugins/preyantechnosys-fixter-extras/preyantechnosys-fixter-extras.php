<?php
/*
 * Plugin Name: Preyantechnosys Extras for Fixter Theme
 * Plugin URI: https://www.preyantechnosys.com
 * Description: Preyantechnosys Plugin for Fixter Theme
 * Version: 1.0
 ...
 */
function preyantechnosys_fixter_cs_framework_init(){
	defined('CS_OPTION'          ) or define('CS_OPTION',           'fixter');
...
add_action( 'init', 'preyantechnosys_fixter_cs_framework_init', 2 );
...
add_action( 'plugins_loaded', 'preyantechnosys_fixter_load_textdomain' );
/**
 * Load plugin textdomain.
 *
 * @since 1.0.0
 */
function preyantechnosys_fixter_load_textdomain() {
 ...
 */
if( !function_exists('preyantechnosys_fixter_scripts_styles') ){
function preyantechnosys_fixter_scripts_styles() {
	wp_enqueue_script( 'jquery-resize', TMTE_URI . '/js/jquery-resize.min.js', array( 'jquery' ) );
}
}
add_action( 'wp_enqueue_scripts', 'preyantechnosys_fixter_scripts_styles' );



if( !function_exists('preyantechnosys_fixter_admin_scripts') ){
function preyantechnosys_fixter_admin_scripts() {
...
add_action( 'admin_enqueue_scripts', 'preyantechnosys_fixter_admin_scripts' );
 ...
 */
if( !function_exists('preyantechnosys_fixter_register_options_page') ){
function preyantechnosys_fixter_register_options_page() {
	add_options_page(
		esc_attr__('Fixter Extra Options', 'tmte'),  // Page title in TITLE tag
		esc_attr__('Fixter Extra Options', 'tmte'),  // heading on page
		'manage_options',
		'tmte-fixter',
		'preyantechnosys_fixter_options_page'
	);
}
}
add_action('admin_menu', 'preyantechnosys_fixter_register_options_page');


/**
 *  Save plugin options
 */
if( !function_exists('preyantechnosys_fixter_register_settings') ){
function preyantechnosys_fixter_register_settings() {
	
	// Social share for Blog
	register_setting( 'tmte_fixter_options_group', 'tmte_fixter_social_share_blog', 'preyantechnosys_fixter_social_share_blog_callback' );
	//add_option( 'tmte_fixter_option_name', 'This is my option value.');
	
	// Social share for Portfolio
	register_setting( 'tmte_fixter_options_group', 'tmte_fixter_social_share_portfolio', 'preyantechnosys_fixter_social_share_portfolio_callback' );
	//add_option( 'tmte_fixter_option_name', 'This is my option value.');
	

}
}
add_action( 'admin_init', 'preyantechnosys_fixter_register_settings' );




if( !function_exists('preyantechnosys_fixter_social_share_blog_callback') ){
function preyantechnosys_fixter_social_share_blog_callback( $data ){
	// Save settings to theme options so we can re-use it
	$fixter_toptions = get_option('fixter_theme_options');
	if( !empty($fixter_toptions['post_social_share_services']) ){
		$fixter_toptions['post_social_share_services'] = $data;
		update_option('fixter_theme_options', $fixter_toptions);
	}
	return $data;
}
}
...
if( !function_exists('preyantechnosys_fixter_social_share_portfolio_callback') ){
function preyantechnosys_fixter_social_share_portfolio_callback( $data ){
	// Save settings to theme options so we can re-use it
	$fixter_toptions = get_option('fixter_theme_options');
	if( !empty($fixter_toptions['portfolio_social_share_services']) ){
		$fixter_toptions['portfolio_social_share_services'] = $data;
		update_option('fixter_theme_options', $fixter_toptions);
	}
	return $data;
}
...
if( !function_exists('preyantechnosys_fixter_options_page') ){
function preyantechnosys_fixter_options_page(){
	...
	?>
	<div class="wrap"> 
		<h1>Fixter Extra Options</h1>
		
		<form method="post" action="options.php">
		
			<?php settings_fields( 'tmte_fixter_options_group' ); ?>

			<p>This page will set some extra options for Fixter theme. So it will be stored even when you change theme.</p>
			<br><br>
			...
			<table class="form-table">
				<tr valign="top">
					<th scope="row"><label for="tmte_fixter_option_name"> Select Social Share Service for Blog Section </label></th>
					<td>
						<p>
						...
						$tmte_fixter_social_share_blog = array();
						if( !empty($fixter_toptions['post_social_share_services']) ){
							$tmte_fixter_social_share_blog = $fixter_toptions['post_social_share_services'];
						...
						foreach( $social_list as $social_name=>$social_slug ){
							$checked = '';
							if( is_array($tmte_fixter_social_share_blog) && in_array( $social_slug, $tmte_fixter_social_share_blog ) ){
								$checked = 'checked="checked"';
							}
							echo '<label><input name="tmte_fixter_social_share_blog[]" type="checkbox" value="'.$social_slug.'" '.$checked.'> ' . $social_name . '</label> <br/>';
						}
				...
				<!-- ---------- -->
				<tr valign="top">
					<th scope="row"><label for="tmte_fixter_option_name"> Select Social Share Service for Portfolio Section </label></th>
					<td>
						<p>
						...
						// Getting from Theme Options
						$tmte_fixter_social_share_portfolio = array();
						if( !empty($fixter_toptions['portfolio_social_share_services']) ){
							$tmte_fixter_social_share_portfolio = $fixter_toptions['portfolio_social_share_services'];
						...
						foreach( $social_list as $social_name=>$social_slug ){
							$checked = '';
							if( is_array($tmte_fixter_social_share_portfolio) && in_array( $social_slug, $tmte_fixter_social_share_portfolio ) ){
								$checked = 'checked="checked"';
							}
							echo '<label><input name="tmte_fixter_social_share_portfolio[]" type="checkbox" value="'.$social_slug.'" '.$checked.'> ' . $social_name . '</label> <br/>';
						}
	...
	if( !empty($post_type) ){		
		...
		if( !empty( $social_services ) && is_array( $social_services ) ){		
			foreach( $social_services as $social ){
				...
				<a href="javascript:void(0)" onClick="TMSocialWindow=window.open(\''. esc_url($link) .'\',\'TMSocialWindow\',width=600,height=100); return false;"><i class="prt-fixter-icon-'. sanitize_html_class($social) .'"></i><span class="cmt-ss">'. $social .'</span></a>
				</li>';
	...
	if( !empty($post_type) ){
		...
		if( !empty( ${ $post_type.'_social_share_services' } ) && is_array( ${$post_type.'_social_share_services'} ) && count( ${$post_type.'_social_share_services'} > 0 ) ){
			foreach( ${$post_type.'_social_share_services'} as $social ){
				...
				// Now preparing the icon
				$return .= '<li class="prt-social-share prt-social-share-'. $social .'">
				<a href="javascript:void(0)" onClick="TMSocialWindow=window.open(\''. esc_url($link) .'\',\'TMSocialWindow\',width=600,height=100); return false;"><i class="prt-fixter-icon-'. sanitize_html_class($social) .'"></i></a>
				</li>';
...
function preyantechnosys_post_set_featured_image_column($columns) {
	...
	foreach( $columns as $key=>$val ){
		$new_columns[$key] = $val;
		if( $key=='title' ){
			$new_columns['preyantechnosys_featured_image'] = esc_attr__( 'Featured Image', 'fixter' );
		}
	}
	return $new_columns;
}
...
function preyantechnosys_author_socials( $contactmethods ) {
	$contactmethods['twitter']  = esc_attr__( 'Twitter Link', 'fixter' );  // Add Twitter
	$contactmethods['facebook'] = esc_attr__( 'Facebook Link', 'fixter' );  //add Facebook
	$contactmethods['linkedin'] = esc_attr__( 'LinkedIn Link', 'fixter' );  //add LinkedIn
	$contactmethods['gplus']    = esc_attr__( 'Google Plus Link', 'fixter' );  //add Google Plus
...
function preyantechnosys_admin_skincolor_css(){
	...
	if( !empty($fixter_theme_options['skincolor']) ){
	?>
	<style>
		.tm_vc_colored-dropdown .skincolor,
		.vc_colored-dropdown .skincolor,
		.vc_btn3.vc_btn3-color-skincolor{  /* VC button */
			background-color: <?php echo esc_attr($fixter_theme_options['skincolor']); ?> !important;
			color: #fff !important;
		}
		.vc_btn3.vc_btn3-color-skincolor.vc_btn3-style-outline{
			color: <?php echo esc_attr($fixter_theme_options['skincolor']); ?> !important;
			border-color: <?php echo esc_attr($fixter_theme_options['skincolor']); ?> !important;
			background-color: transparent !important;
		}
		.vc_btn3.vc_btn3-color-skincolor.vc_btn3-style-3d {
			box-shadow: 0 4px rgba(<?php echo preyantechnosys_hex2rgb($fixter_theme_options['skincolor']); ?>, 0.73), 0 4px rgb(0, 0, 0) !important;
		}
...
function preyantechnosys_login_page_css() {
	...
	if( isset($fixter_theme_options['login_custom_css_code']) && trim($fixter_theme_options['login_custom_css_code'])!='' ){
		$return .= $fixter_theme_options['login_custom_css_code'];
	}
	
	// Login page background
	$return .= preyantechnosys_get_background_css('body.login', $fixter_theme_options['login_background']);
	...
	if( !empty($fixter_theme_options['logoimg']) ){
		
		if( !empty($fixter_theme_options['logoimg']['full-url']) ){
			
			$image = $fixter_theme_options['logoimg']['full-url'];  // Image src
			
			if( function_exists('getimagesize') ){
				$imgsize_array = getimagesize( $fixter_theme_options['logoimg']['full-url'] );
	...
	// Login button
	if( !empty($fixter_theme_options['skincolor']) ){
		$return .= '#wp-submit{background-color:'. $fixter_theme_options['skincolor'] .'}';
	}
 ...
 * @return void
 */
function fixter_widgets_init() {
	register_sidebar( array(
			'name' => esc_attr__( 'Header Right Box Widget', 'fixter' ),
			'id' => 'header-rightbox-widget',
			'description' => esc_attr__( 'This is header right box widget', 'fixter' ),
	...
	register_sidebar( array(
			'name' => esc_attr__( 'Slider Menu Box Widget', 'fixter' ),
			'id' => 'slider-menu-widget',
			'description' => esc_attr__( 'This is Slider menu box widget', 'fixter' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Elementor Search', 'fixter' ),
		'id' => 'elementor-searchbox',
		'description' => esc_attr__( 'This is elementor search for elementor', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Left Sidebar for Blog', 'fixter' ),
		'id' => 'sidebar-left-blog',
		'description' => esc_attr__( 'This is left sidebar for blog section', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Left Sidebar for Pages', 'fixter' ),
		'id' => 'sidebar-left-page',
		'description' => esc_attr__( 'This is left sidebar for pages', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	// Portfolio - Left
	register_sidebar( array(
		'name' => esc_attr__( 'Left Sidebar for Portfolio', 'fixter' ),
		'id' => 'sidebar-left-portfolio',
		'description' => esc_attr__( 'This is left sidebar for Portfolio', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	// Portfolio - Right
	register_sidebar( array(
		'name' => esc_attr__( 'Right Sidebar for Portfolio', 'fixter' ),
		'id' => 'sidebar-right-portfolio',
		'description' => esc_attr__( 'This is right sidebar for Portfolio', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Left Sidebar for Portfolio Category', 'fixter' ),
		'id' => 'sidebar-left-portfoliocat',
		'description' => esc_attr__( 'This is left sidebar for Portfolio Category pages.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Right Sidebar for Portfolio Category', 'fixter' ),
		'id' => 'sidebar-right-portfoliocat',
		'description' => esc_attr__( 'This is right sidebar for Portfolio Category pages.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
		...
		// Service - Left
	register_sidebar( array(
		'name' => esc_attr__( 'Left Sidebar for Service', 'fixter' ),
		'id' => 'sidebar-left-service',
		'description' => esc_attr__( 'This is left sidebar for Service', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	// Service - Right
	register_sidebar( array(
		'name' => esc_attr__( 'Right Sidebar for Service', 'fixter' ),
		'id' => 'sidebar-right-service',
		'description' => esc_attr__( 'This is right sidebar for Service', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Left Sidebar for Service Category', 'fixter' ),
		'id' => 'sidebar-left-servicecat',
		'description' => esc_attr__( 'This is left sidebar for Service Category pages.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Right Sidebar for Service Category', 'fixter' ),
		'id' => 'sidebar-right-servicecat',
		'description' => esc_attr__( 'This is right sidebar for Service Category pages.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	// Team Member - Left
	register_sidebar( array(
		'name' => esc_attr__( 'Left Sidebar for Team Member', 'fixter' ),
		'id' => 'sidebar-left-team-member',
		'description' => esc_attr__( 'This is left sidebar for Team Member', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	// Team Member - Right
	register_sidebar( array(
		'name' => esc_attr__( 'Right Sidebar for Team Member', 'fixter' ),
		'id' => 'sidebar-right-team-member',
		'description' => esc_attr__( 'This is right sidebar for Team Member', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Left Sidebar for Team Member Group pages', 'fixter' ),
		'id' => 'sidebar-left-team-member-group',
		'description' => esc_attr__( 'This is left sidebar for Team Member Group.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Right Sidebar for Team Member Group pages', 'fixter' ),
		'id' => 'sidebar-right-team-member-group',
		'description' => esc_attr__( 'This is right sidebar for Team Member Group', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Left Sidebar for Search', 'fixter' ),
		'id' => 'sidebar-left-search',
		'description' => esc_attr__( 'This is left sidebar for search', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Right Sidebar for search', 'fixter' ),
		'id' => 'sidebar-right-search',
		'description' => esc_attr__( 'This is right sidebar for search', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	if( function_exists('is_woocommerce') ){
		// WooCommerce - Left
		register_sidebar( array(
			'name' => esc_attr__( 'Left Sidebar for WooCommerce Shop', 'fixter' ),
			'id' => 'sidebar-left-woocommerce',
			'description' => esc_attr__( 'This is left sidebar for WooCommerce shop pages.', 'fixter' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>',
		...
		// WooCommerce - Right
		register_sidebar( array(
			'name' => esc_attr__( 'Right Sidebar for WooCommerce Shop', 'fixter' ),
			'id' => 'sidebar-right-woocommerce',
			'description' => esc_attr__( 'This is right sidebar for WooCommerce shop pages.', 'fixter' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>',
	...
	if( function_exists('is_bbpress') ){
		// BBPress - Left
		register_sidebar( array(
			'name'          => esc_attr__( 'Left Sidebar for BBPress', 'fixter' ),
			'id'            => 'sidebar-left-bbpress',
			'description'   => esc_attr__( 'This is left sidebar for BBPress.', 'fixter' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget'  => '</aside>',
			'before_title'  => '<h3 class="widget-title">',
			'after_title'   => '</h3>',
		...
		// BBPress - Right
		register_sidebar( array(
			'name'          => esc_attr__( 'Right Sidebar for BBPress', 'fixter' ),
			'id'            => 'sidebar-right-bbpress',
			'description'   => esc_attr__( 'This is right sidebar for BBPress.', 'fixter' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget'  => '</aside>',
			'before_title'  => '<h3 class="widget-title">',
			'after_title'   => '</h3>',
	...
	if( function_exists('tribe_is_upcoming') ){
		...
		register_sidebar( array(
			'name'          => esc_attr__( 'Left Sidebar for Events', 'fixter' ),
			'id'            => 'sidebar-left-events',
			'description'   => esc_attr__( 'This is left sidebar for "The Events Calendar" plugin only.', 'fixter' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget'  => '</aside>',
			'before_title'  => '<h3 class="widget-title">',
			'after_title'   => '</h3>',
		...
		register_sidebar( array(
			'name'          => esc_attr__( 'Right Sidebar for Events', 'fixter' ),
			'id'            => 'sidebar-right-events',
			'description'   => esc_attr__( 'This is right sidebar for "The Events Calendar" plugin only.', 'fixter' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget'  => '</aside>',
			'before_title'  => '<h3 class="widget-title">',
			'after_title'   => '</h3>',
	...
	register_sidebar( array(
		'name'          => esc_attr__( 'Floatingbar Widget - Top', 'fixter' ),
		'id'            => 'floating-widgets-top',
		'description'   => esc_attr__( 'This widget will appear (as full width) before the widget columns. So you can set any full width content here.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h3 class="widget-title">',
		'after_title'   => '</h3>',
	...
	register_sidebar( array(
		'name'          => esc_attr__( 'Floatingbar Widget - 1st column', 'fixter' ),
		'id'            => 'floating-widgets-1',
		'description'   => esc_attr__( 'Set 1st column widgets for Floatingbar area.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h3 class="widget-title">',
		'after_title'   => '</h3>',
	...
	register_sidebar( array(
		'name'          => esc_attr__( 'Floatingbar Widget - 2nd column', 'fixter' ),
		'id'            => 'floating-widgets-2',
		'description'   => esc_attr__( 'Set 2nd column widgets for Floatingbar area.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h3 class="widget-title">',
		'after_title'   => '</h3>',
	...
	register_sidebar( array(
		'name'          => esc_attr__( 'Floatingbar Widget - 3rd column', 'fixter' ),
		'id'            => 'floating-widgets-3',
		'description'   => esc_attr__( 'Set 3rd column widgets for Floatingbar area.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h3 class="widget-title">',
		'after_title'   => '</h3>',
	...
	register_sidebar( array(
		'name'          => esc_attr__( 'Floatingbar Widget - 4th column', 'fixter' ),
		'id'            => 'floating-widgets-4',
		'description'   => esc_attr__( 'Set 4th column widgets for Floatingbar area.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h3 class="widget-title">',
		'after_title'   => '</h3>',
	...
	register_sidebar( array(
		'name'          => esc_attr__( 'Floatingbar Widget - Bottom', 'fixter' ),
		'id'            => 'floating-widgets-bottom',
		'description'   => esc_attr__( 'This widget will appear (as full width) after the widget columns. So you can set any full width content here.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h3 class="widget-title">',
		'after_title'   => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'First Footer - 1st Widget Area', 'fixter' ),
		'id' => 'first-footer-1-widget-area',
		'description' => esc_attr__( 'This is first footer widget area for first row of footer.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'First Footer - 2nd Widget Area', 'fixter' ),
		'id' => 'first-footer-2-widget-area',
		'description' => esc_attr__( 'This is second footer widget area for first row of footer.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'First Footer - 3rd Widget Area', 'fixter' ),
		'id' => 'first-footer-3-widget-area',
		'description' => esc_attr__( 'This is third footer widget area for first row of footer.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'First Footer - 4th Widget Area', 'fixter' ),
		'id' => 'first-footer-4-widget-area',
		'description' => esc_attr__( 'This is fourth footer widget area for first row of footer.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Second Footer - 1st Widget Area', 'fixter' ),
		'id' => 'second-footer-1-widget-area',
		'description' => esc_attr__( 'This is first footer widget area for second row of footer.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Second Footer - 2nd Widget Area', 'fixter' ),
		'id' => 'second-footer-2-widget-area',
		'description' => esc_attr__( 'This is second footer widget area for second row of footer.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Second Footer - 3rd Widget Area', 'fixter' ),
		'id' => 'second-footer-3-widget-area',
		'description' => esc_attr__( 'This is third footer widget area for second row of footer.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Second Footer - 4th Widget Area', 'fixter' ),
		'id' => 'second-footer-4-widget-area',
		'description' => esc_attr__( 'This is fourth footer widget area for second row of footer.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => esc_attr__( 'Second Footer - 5th Widget Area', 'fixter' ),
		'id' => 'second-footer-5-widget-area',
		'description' => esc_attr__( 'This is fourth footer widget area for second row of footer.', 'fixter' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
	...
	global $fixter_theme_options;
	$fixter_theme_options = get_option('fixter_theme_options');
	if( isset($fixter_theme_options['custom_sidebars']) && is_array($fixter_theme_options['custom_sidebars']) && count($fixter_theme_options['custom_sidebars'])>0 ){
		foreach( $fixter_theme_options['custom_sidebars'] as $custom_sidebar ){
			...
			if( isset($custom_sidebar['custom_sidebar']) && trim($custom_sidebar['custom_sidebar'])!='' ){
				...
				if( trim($custom_sidebar)!='' ){
					...
					register_sidebar( array(
						...
						'description'   => esc_attr__( 'This is custom widget developed from "Fixter Options".', 'fixter' ),
						'before_widget' => '<aside id="%1$s" class="widget %2$s">',
						'after_widget'  => '</aside>',
						'before_title'  => '<h3 class="widget-title">',
						'after_title'   => '</h3>',
...
add_action( 'widgets_init', 'fixter_widgets_init' );
