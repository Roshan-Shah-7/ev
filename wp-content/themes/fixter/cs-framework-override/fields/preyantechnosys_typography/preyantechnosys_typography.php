...
class CSFramework_Option_preyantechnosys_typography extends CSFramework_Options {
  ...
  public function output() {
    ...
    if( is_object( $google_json ) ) {
	  ...
	  echo '<small>'.esc_attr__( 'Font Family', 'fixter' ).'</small><br>';
      ...
      echo '<optgroup label="'. esc_attr__( 'Web Safe Fonts', 'fixter' ) .'">';
      ...
      echo '</optgroup>';
      echo '<optgroup label="'. esc_attr__( 'Google Fonts', 'fixter' ) .'">';
	   ...
	   */
		if( ! empty( $is_backup_family ) ) {
			...
			echo '<small>'.esc_attr__( 'Backup Font Family', 'fixter' ).'</small><br>';
      ...
      if( ! empty( $is_variant ) ) {
		...
		echo '<small>'.esc_attr__( 'Font Variants', 'fixter' ).'</small><br>';
		...
		if( ! empty( $is_text_transform ) ) {
			...
			echo '<small>'.esc_attr__( 'Text Transform', 'fixter' ).'</small><br>';
	   ...
	   echo '<div class="clear clr"></div>  </div> <!-- .cs-prt-typography-row2 -->';
		...
		if( ! empty( $is_font_size ) ) {
			...
			echo '<small>'.esc_attr__( 'Font Size', 'fixter' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-font-size mini" min="1" max="200" step="1" title="'.esc_attr__( 'Font Size', 'fixter' ).'" name="'. esc_attr($this->element_name( '[font-size]' )) .'" value="'.esc_attr($font_size_value).'">
		...
		if( ! empty( $is_line_height ) ) {
			...
			echo '<small>'.esc_attr__( 'Line Height', 'fixter' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-line-height mini" min="1" max="200" step="1" title="'.esc_attr__( 'Line Height', 'fixter' ).'" name="'. esc_attr($this->element_name( '[line-height]' )) .'" value="'.esc_attr($line_height_value).'">
			<span class="add-on">px</span>
			<div class="clear clr"></div>
			</div><div class="clear clr"></div>';
			echo '</label>';
		...
		if( ! empty( $is_letter_spacing ) ) {
			echo '<div class="prt-fancy-input-w">';
			echo '<label class="cs-typography-line-height">';
			echo '<small>'.esc_attr__( 'Letter Spacing', 'fixter' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-line-height mini" min="0" max="20" step="0.1" title="'.esc_attr__( 'Letter Spacing', 'fixter' ).'" name="'. esc_attr($this->element_name( '[letter-spacing]' )) .'" value="'.esc_attr($letter_spacing_value).'">
			<span class="add-on">px</span>
			<div class="clear clr"></div>
			</div><div class="clear clr"></div>';
			echo '</label>';
		...
		if( ! empty( $is_color ) ) {
			...
			echo '<small>'.esc_attr__( 'Font Color', 'fixter' ).'</small><br>';
      ...
      if( ! empty( $is_all_varients ) ) {
		...
		echo '<div class="prt-typo-load-all-varient-text">'. esc_attr__('Load all variations of the selected font (Like bold, italic etc.)','fixter') .'<br><small>'. esc_attr__('Loading all font variations can slow down (a little) your webpage.','fixter').'</small></div>';
		echo '<div class="clear clr"></div>';
        echo '</label>';
		echo '</div>';
      }
      ...
      echo esc_attr__( 'Error! Can not load json file.', 'fixter' );
